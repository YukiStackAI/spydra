...
Most crawls fall into one of two patterns: "follow links matching this regex" or "crawl every URL listed in the site's sitemap". Spydra ships templates for both so you don't have to hand-write the same `parse()` boilerplate every time.
...
```python
from spydra.spiders import SitemapSpider, CrawlRule, LinkExtractor
...
```python
from spydra.spiders import Spider, LinkExtractor
