# Spydra Extract Command Guide

**Web Scraping through the terminal without requiring any programming!**

The `spydra extract` command lets you download and extract content from websites directly from your terminal without writing any code. Ideal for beginners, researchers, and anyone requiring rapid web data extraction.
...
- **Basic Website Download**
    ...
    ```bash
    spydra extract get "https://example.com" page_content.txt
    ```
...
- **Save as Different Formats**
    ...
    spydra extract get "https://blog.example.com" article.md
    
    # Save the HTML content as it is to the file
    spydra extract get "https://example.com" page.html
    
    # Save a clean version of the text content of the webpage to the file
    spydra extract get "https://example.com" content.txt
  
    # Or use the Docker image with something like this:
    docker run -v $(pwd)/output:/output spydra extract get "https://blog.example.com" /output/article.md 
    ```
...
You can display the available commands through `spydra extract --help` to get the following list:
```bash
...
1. **GET Request**
    ...
    ```bash
    spydra extract get [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    ...
    **Examples:**
    ...
    ```
    Get the available options for the command with `spydra extract get --help` as follows:
    ```bash
    Usage: spydra extract get [OPTIONS] URL OUTPUT_FILE
...
2. **Post Request**
    ...
    ```bash
    spydra extract post [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Submit form data
    spydra extract post "https://api.site.com/search" results.html --data "query=python&type=tutorial"
    
    # Send JSON data
    spydra extract post "https://api.site.com" response.json --json '{"username": "test", "action": "search"}'
    ```
    Get the available options for the command with `spydra extract post --help` as follows:
    ```bash
    Usage: spydra extract post [OPTIONS] URL OUTPUT_FILE
...
3. **Put Request**
    ...
    ```bash
    spydra extract put [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Send data
    spydra extract put "https://spydra.requestcatcher.com/put" results.html --data "update=info" --impersonate "firefox"
    
    # Send JSON data
    spydra extract put "https://spydra.requestcatcher.com/put" response.json --json '{"username": "test", "action": "search"}'
    ```
    Get the available options for the command with `spydra extract put --help` as follows:
    ```bash
    Usage: spydra extract put [OPTIONS] URL OUTPUT_FILE
...
4. **Delete Request**
    ...
    ```bash
    spydra extract delete [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Send data
    spydra extract delete "https://spydra.requestcatcher.com/delete" results.html
    
    # Send JSON data
    spydra extract delete "https://spydra.requestcatcher.com/" response.txt --impersonate "chrome"
    ```
    Get the available options for the command with `spydra extract delete --help` as follows:
    ```bash
    Usage: spydra extract delete [OPTIONS] URL OUTPUT_FILE
...
1. **fetch - Handle Dynamic Content**
    ...
    ```bash
    spydra extract fetch [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    ...
    **Examples:**
    ...
    ```
    Get the available options for the command with `spydra extract fetch --help` as follows:
    ```bash
    Usage: spydra extract fetch [OPTIONS] URL OUTPUT_FILE
...
2. **stealthy-fetch - Bypass Protection**
    ...
    ```bash
    spydra extract stealthy-fetch [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    ...
    **Examples:**
    ...
    ```
    Get the available options for the command with `spydra extract stealthy-fetch --help` as follows:
    ```bash
    Usage: spydra extract stealthy-fetch [OPTIONS] URL OUTPUT_FILE
