...
When scraping at scale, you'll often need to rotate through multiple proxies to avoid rate limits and blocks. Spydra's `ProxyRotator` makes this straightforward. It works with all session types and integrates with the spider's blocked request retry system.
...
from spydra.fetchers import FetcherSession, ProxyRotator
...
```python
from spydra.fetchers import AsyncDynamicSession, AsyncStealthySession, ProxyRotator
...
```python
from spydra.core._types import ProxyType
...
```python
from spydra.spiders import Spider, SessionManager, Request, Response
...
```python
from spydra.spiders import Spider, SessionManager, Request, Response
from spydra.fetchers import FetcherSession, AsyncStealthySession, ProxyRotator
