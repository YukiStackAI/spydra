...
<p align="center">
    <a href="https://spydra.readthedocs.io/en/latest/parsing/selection.html"><strong>선택 메서드</strong></a>
    &middot;
    <a href="https://spydra.readthedocs.io/en/latest/fetching/choosing.html"><strong>Fetcher 선택 가이드</strong></a>
    &middot;
    <a href="https://spydra.readthedocs.io/en/latest/spiders/architecture.html"><strong>Spider</strong></a>
    &middot;
    <a href="https://spydra.readthedocs.io/en/latest/spiders/proxy-blocking.html"><strong>프록시 로테이션</strong></a>
    &middot;
    <a href="https://spydra.readthedocs.io/en/latest/cli/overview.html"><strong>CLI</strong></a>
    &middot;
    <a href="https://spydra.readthedocs.io/en/latest/ai/mcp-server.html"><strong>MCP 서버</strong></a>
</p>

Spydra은 단일 요청부터 대규모 크롤링까지 모든 것을 처리하는 적응형 Web Scraping 프레임워크입니다.
...
<table>
  ...
  <tr>
    ...
    <td> Spydra은 Cloudflare Turnstile을 처리합니다. 엔터프라이즈급 보호가 필요하다면, <a href="https://hypersolutions.co?utm_source=github&utm_medium=readme&utm_campaign=spydra">
        <b>Hyper Solutions</b>
  ...
  <tr>
    ...
    <td>프록시는 복잡하거나 비쌀 이유가 없다는 생각으로 <a href="https://birdproxies.com/t/spydra">
        <b>BirdProxies</b>
  ...
  <tr>
    ...
    <td>
    <a href="https://substack.thewebscraping.club/p/spydra-hands-on-guide?utm_source=github&utm_medium=repo&utm_campaign=spydra" target="_blank">The Web Scraping Club에서 Spydra의 전체 리뷰</a>(2025년 11월)를 읽어보세요. 웹 스크래핑 전문 No.1 뉴스레터입니다.
    </td>
  </tr>
  <tr>
    <td width="200">
...
- 🤖 **AI와 함께 사용하는 MCP 서버**: AI 기반 Web Scraping과 데이터 추출을 위한 내장 MCP 서버. AI(Claude/Cursor 등)에 전달하기 전에 Spydra을 활용해 대상 콘텐츠를 추출하는 강력한 커스텀 기능을 갖추고 있어, 작업 속도를 높이고 토큰 사용량을 최소화해 비용을 절감합니다. ([데모 영상](https://www.youtube.com/watch?v=qyFk3ZNwOxE))

### 고성능 & 실전 검증된 아키텍처
- 🚀 **초고속**: 대부분의 Python 스크레이핑 라이브러리를 능가하는 최적화된 성능.
- 🔋 **메모리 효율**: 최적화된 데이터 구조와 지연 로딩으로 메모리 사용을 최소화합니다.
- ⚡ **고속 JSON 직렬화**: 표준 라이브러리보다 10배 빠릅니다.
- 🏗️ **실전 검증**: Spydra은 92%의 테스트 커버리지와 완전한 타입 힌트 커버리지를 갖추고 있을 뿐 아니라, 지난 1년간 수백 명의 Web Scraper가 매일 사용해 왔습니다.

### 개발자/Web Scraper 친화적 경험
- 🎯 **인터랙티브 Web Scraping Shell**: Spydra 통합, 단축키, curl 요청을 Spydra 요청으로 변환하거나 브라우저에서 요청 결과를 확인하는 등의 도구를 갖춘 선택적 내장 IPython Shell로, Web Scraping 스크립트 개발을 가속합니다.
- 🚀 **터미널에서 바로 사용**: 코드 한 줄 없이 Spydra으로 URL을 스크레이핑할 수 있습니다!
...
깊이 들어가지 않고, Spydra이 할 수 있는 것들을 간단히 살펴보겠습니다.
...
Spydra에는 강력한 커맨드라인 인터페이스가 포함되어 있습니다:
...
spydra extract get 'https://example.com' content.txt --css-selector '#fromSkipToProducts' --impersonate 'chrome'  # CSS selector '#fromSkipToProducts'에 매칭되는 모든 요소
...
> [!NOTE]
> MCP 서버와 인터랙티브 Web Scraping Shell 등 더 많은 기능이 있지만, 이 페이지는 간결하게 유지하겠습니다. 전체 문서는 [여기](https://spydra.readthedocs.io/en/latest/)에서 확인하세요.

## 성능 벤치마크

Spydra은 강력할 뿐만 아니라 초고속입니다. 아래 벤치마크는 Spydra의 파서를 다른 인기 라이브러리의 최신 버전과 비교한 것입니다.
...
Spydra의 적응형 요소 찾기 기능은 대안들을 크게 앞섭니다:
...
> 모든 벤치마크는 100회 이상 실행의 평균입니다. 측정 방법은 [benchmarks.py](https://github.com/D4Vinci/Spydra/blob/main/benchmarks.py)를 참조하세요.

## 설치

Spydra은 Python 3.10 이상이 필요합니다:
...
1. 아래의 추가 기능, Fetcher, 또는 관련 클래스를 사용하려면 Fetcher 의존성과 브라우저 의존성을 다음과 같이 설치해야 합니다:
    ...
    spydra install           # 일반 설치
    spydra install  --force  # 강제 재설치
    ```
...
2. 추가 기능:
   ...
   - 모든 기능 설치:
       ...
       ```
   위 추가 기능을 설치한 후에도 (아직 하지 않았다면) `spydra install`로 브라우저 의존성을 설치해야 합니다.
...
기여를 환영합니다! 시작하기 전에 [기여 가이드라인](https://github.com/D4Vinci/Spydra/blob/main/CONTRIBUTING.md)을 읽어주세요.
...
- Parsel (BSD 라이선스) - [translator](https://github.com/D4Vinci/Spydra/blob/main/spydra/core/translator.py) 서브모듈에 사용
